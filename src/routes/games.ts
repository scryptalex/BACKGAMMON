...
import { generateInviteCode, isValidStake } from '../utils/validation';
import { initializeBoard } from '../services/gameLogic';
...
router.get('/', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (type && ['short', 'long'].includes(type as string)) {
      filter.type = type;
    }
    ...
    if (minStake || maxStake) {
      filter.stake = {};
      if (minStake) (filter.stake as Record<string, number>).$gte = Number(minStake);
      if (maxStake) (filter.stake as Record<string, number>).$lte = Number(maxStake);
    }
...
router.get('/invite/:code', auth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const code = String(req.params.code);

    const game = await Game.findOne({ inviteCode: code.toUpperCase(), status: 'waiting' })
      .populate('player1', 'name walletAddress');
...
// Get user's games
router.get('/user/my-games', auth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ error: 'Authentication required' });
      return;
