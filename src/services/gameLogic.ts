import { BoardState, GameType, Move } from '../models/Game';
 ...
 */
export function initializeBoard(type: GameType): BoardState {
 ...
 */
export function getAvailableMoves(dice: [number, number]): number[] {
  if (dice[0] === dice[1]) {
    // Doubles - 4 moves
...
export function isValidMoveShort(
  ...
  if (from === -1) {
    if (player === 1) {
      ...
      if (to !== enterPoint) return false;
      // Check if blocked (2+ opponent checkers)
      if (positions[to] <= -2) return false;
    } else {
  ...
  if (player === 1 && positions[from] <= 0) return false;
  if (player === 2 && positions[from] >= 0) return false;
  
  // Check if destination is blocked
  if (player === 1 && positions[to] <= -2) return false;
  if (player === 2 && positions[to] >= 2) return false;
...
export function isValidMoveLong(
  ...
  if (player === 1 && positions[to] < 0) return false;
  if (player === 2 && positions[to] > 0) return false;
 ...
 */
export function canPlayerBearOff(boardState: BoardState, player: 1 | 2): boolean {
  const { positions, player1Bar, player2Bar } = boardState;
  
  if (player === 1) {
    if (player1Bar > 0) return false;
...
export function applyMove(
  ...
  const newState = JSON.parse(JSON.stringify(boardState)) as BoardState;
  
  // Remove checker from origin
  if (from === -1) {
    // From bar
    if (player === 1) newState.player1Bar--;
    else newState.player2Bar--;
  } else {
    if (player === 1) newState.positions[from]--;
    else newState.positions[from]++;
  }
  
  // Handle bearing off
  if (to === 24 || to < 0) {
    if (player === 1) newState.player1Off++;
    else newState.player2Off++;
  } else {
    // Regular move
    if (gameType === 'short') {
    ...
    if (player === 1) newState.positions[to]++;
    else newState.positions[to]--;
  }
 ...
 */
export function checkWinner(boardState: BoardState): 1 | 2 | null {
  if (boardState.player1Off === 15) return 1;
  if (boardState.player2Off === 15) return 2;
  return null;
}
 ...
 */
export function switchPlayer(currentPlayer: 1 | 2): 1 | 2 {
